...
# Shared importance level mapping used across the codebase
IMPORTANCE_LEVELS = {
    "low": 3,
    "medium": 5,
    "high": 8,
...
class ScoringEngine:
    ...
    def compute(self, memory: Dict[str, Any], now: Optional[datetime] = None) -> int:
        ...
        importance_level = IMPORTANCE_LEVELS.get(importance_text, 5)
...
# Custom rule factory functions

def create_topic_boost_rule(topics: list[str], boost_amount: float = 10.0) -> Callable[[Dict[str, Any]], float]:
    """Create a rule that boosts score for specific topics.

    Args:
        topics: List of topic strings to boost
        boost_amount: Amount to add to score (default 10.0)

    Returns:
        Callable rule function
    """
    def rule(memory: Dict[str, Any]) -> float:
...
def create_stale_penalty_rule(threshold_days: int = 180, penalty_amount: float = -15.0) -> Callable[[Dict[str, Any]], float]:
    """Create a rule that penalizes very old memories.

    Args:
        threshold_days: Age in days beyond which to penalize
        penalty_amount: Amount to subtract from score (default -15.0)
...
def create_entity_match_rule(entities: list[str], boost_amount: float = 5.0) -> Callable[[Dict[str, Any]], float]:
    """Create a rule that boosts score if memory content contains specific entities.

    Args:
        entities: List of entity strings to search for (case-insensitive)
        boost_amount: Amount to add to score per match (default 5.0)
    ...
    def rule(memory: Dict[str, Any]) -> float:
        ...
        # Check content and metadata for entities
        matches = 0
        for entity in entities:
            if entity.lower() in content:
                matches += 1
            # Also check if entity is in metadata values
            for value in metadata.values():
                if isinstance(value, str) and entity.lower() in value.lower():
                    matches += 1
                    break

        return matches * boost_amount
    return rule
